...
def Q_1(conn, execution_time):
    ...
    query = """SELECT AVG(s.statsbomb_xg), p.player_name FROM shot s
            INNER JOIN events e ON e.type_id = s.type_id
            INNER JOIN matches m ON m.match_id = e.match_id
            INNER JOIN competition c ON c.season_id = m.season_id
...
def Q_2(conn, execution_time):
    ...
    query = """SELECT p.player_name, count(s.statsbomb_xg) FROM shot s
            INNER JOIN events e ON e.type_id = s.type_id
            INNER JOIN matches m ON m.match_id = e.match_id
            INNER JOIN competition c ON c.season_id = m.season_id
            INNER JOIN player p ON p.player_id = e.player_id
            WHERE c.competition_name='La Liga' and c.season_name='2020/2021'
            GROUP BY (p.player_name)
            ORDER BY count(s.statsbomb_xg) DESC;"""
...
def Q_3(conn, execution_time):
    ...
    query = """SELECT p.player_name, count(s.first_time) FROM shot s
            INNER JOIN events e ON e.type_id = s.type_id
            INNER JOIN matches m ON m.match_id = e.match_id
            INNER JOIN competition c ON c.season_id = m.season_id
            INNER JOIN player p ON p.player_id = e.player_id
...
def Q_4(conn, execution_time):
    ...
    query = """SELECT t.team_name, count(*) 
            FROM team t
            INNER JOIN events e ON e.team_id = t.team_id
            INNER JOIN matches m ON m.match_id = e.match_id
            INNER JOIN competition c ON c.season_id = m.season_id
            WHERE c.competition_name='La Liga' and c.season_name='2020/2021' and e.type_name = 'Pass'
...
def Q_5(conn, execution_time):
    ...
    query = """SELECT p.player_name, count(*) FROM pass ps
            INNER JOIN player p ON ps.recipient_id = p.player_id
            INNER JOIN events e ON e.player_id = p.player_id
            INNER JOIN matches m ON m.match_id = e.match_id
            INNER JOIN competition c ON c.season_id = m.season_id
...
def Q_6(conn, execution_time):
    ...
    query = """SELECT t.team_name, count(*) FROM team t
        INNER JOIN events e ON e.team_id = t.team_id
        INNER JOIN matches m ON m.match_id = e.match_id
        INNER JOIN competition c ON c.season_id = m.season_id
        WHERE c.competition_name='Premier League' and c.season_name='2003/2004' and e.type_name = 'Shot'
        GROUP BY (t.team_name)
        ORDER BY count(*) DESC;"""
...
def Q_7(conn, execution_time):
    ...
    query = """SELECT p.player_name, count(ps.through_ball) FROM pass ps
INNER JOIN events e ON e.type_id = ps.type_id
INNER JOIN matches m ON m.match_id = e.match_id
INNER JOIN competition c ON c.season_id = m.season_id
INNER JOIN player p ON p.player_id = e.player_id
WHERE c.competition_name='La Liga' and c.season_name='2020/2021' and ps.through_ball='True'
GROUP BY (p.player_name)
ORDER BY count(ps.through_ball) DESC;"""
...
def Q_8(conn, execution_time):
    ...
    query = """SELECT t.team_name, count(ps.through_ball) FROM pass ps
INNER JOIN events e ON e.type_id = ps.type_id
INNER JOIN matches m ON m.match_id = e.match_id
INNER JOIN competition c ON c.season_id = m.season_id
...
def Q_9(conn, execution_time):
    ...
    query = """SELECT p.player_name, count(*) FROM events e
INNER JOIN matches m ON m.match_id = e.match_id
INNER JOIN competition c ON c.season_id = m.season_id
INNER JOIN player p ON p.player_id = e.player_id
WHERE c.competition_name='La Liga' and e.dribble_type='Complete'
GROUP BY (p.player_name)
ORDER BY count(*) DESC;"""
...
INNER JOIN competition c ON c.season_id = m.season_id
INNER JOIN player p ON p.player_id = e.player_id
WHERE c.competition_name='La Liga' and c.season_name='2020/2021' and e.dribble_type='Complete'
GROUP BY (p.player_name)
ORDER BY count(*) ASC;"""
